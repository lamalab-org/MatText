...
from commons import MODEL_STYLES, NON_MATTEXT_REPS, range_frame, save_figure
...
def download_data(
    ...
    logger.info(f"Downloading data from {repo_id}...")
...
def plot_panel_c(averaged: dict, output_file: str):
    ...
    png_path, pdf_path = save_figure(fig, output_file.replace(".png", ""))
    logger.success(f"Saved panel C: {png_path} and {pdf_path}")
    plt.close(fig)
...
def plot_appendix(per_dataset: dict, output_file: str, single_row: bool = False):
    ...
    plt.tight_layout()
    png_path, pdf_path = save_figure(fig, output_file.replace(".png", ""))
    logger.success(f"Saved appendix figure: {png_path} and {pdf_path}")
    plt.close(fig)
...
def main():
    ...
    logger.info(f"Datasets found: {list(per_dataset.keys())}")

    averaged = compute_averaged_values(per_dataset)
    logger.info("Averaged contributions across datasets:")
    ...
    plot_panel_c(averaged, "figure_2_panel_c")
    plot_appendix(per_dataset, "figure_2_appendix_2row", single_row=False)
    plot_appendix(per_dataset, "figure_2_appendix_1row", single_row=True)
