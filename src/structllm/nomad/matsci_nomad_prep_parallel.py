...
def prep_data(lmdb_path: str, output_file: str, num_processes: int = 24) -> None:
    materials_list = []
    dataset = Dataset(lmdb_path, 1)
    total_entries = dataset.len()

    with Pool(processes=num_processes) as pool:
        results = list(tqdm(pool.imap(process_data, range(total_entries)), total=total_entries))

    materials_list.extend(results)
