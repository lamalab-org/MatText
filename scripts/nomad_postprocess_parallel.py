...
def create_cif(mat_dict: dict):
    ...
    lattice_params = crystal["lattice_parameters"]
...
def prepare_dict(mat_dict: dict):
    ...
    mass_density = mat_dict["properties"]["structures"]["structure_original"].get(
        "mass_density", None
    )
    ...
    try:
        ...
        electronic = {
            ...
            "energy_highest_occupied": elec_structure["band_gap"][0].get(
                "energy_highest_occupied", None
            ),
            "energy_lowest_unoccupied": elec_structure["band_gap"][0].get(
...
def prep_data(lmdb_path: str, output_file: str, num_processes: int = 4) -> None:
    ...
    with Pool(processes=num_processes) as pool:
        results = list(
            tqdm(
                pool.imap(process_data, [(lmdb_path, i) for i in range(total_entries)]),
                total=total_entries,
            )
        )
    ...
    print(
        f"Total number of skipped datapoints: {skipped_count}"
    )  # Print the total number of skipped datapoints
