        ...
        for sp in ["train", "test"]:
            for fold in range(5):
                report_files = glob.glob(
                    f"{reports_path}/{sp}_{prop}_{fold}.json_report.json"
                )
                if len(report_files) > 0:
                    mbids = get_bid_mbid(report_files[0])

                    print(
                        f"No of big structures in {sp}_{prop}_{fold} are : ", len(mbids)
                    )
                    ...
                    matbench_data["splits"][prop][fold_key][sp] = [
                        id for id in ids if id not in mbids
                    ]
