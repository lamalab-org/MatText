...
def filter_dataset_with_context(
    ...
    for representation, context_length in representation_context.items():
        ...
        def filter_func(example):
            tokens = _tokenize_without_truncation(
                {"representation": example[representation]}, representation
            )
...
def process_datasets(
    ...
    for filename in os.listdir(input_dir):
        if filename.endswith(".json"):
            ...
            filtered_dataset, report = filter_dataset_with_context(
                dataset, representation_context
            )
