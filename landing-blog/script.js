...
(() => {
  ...
  /* ------------------------------------------------------------------ *
   * Figure 2 — CC-Cliff, driven by real Figure 2 source data
   * (error_geom_{alpha} columns, min–max normalized per property, then
   * averaged across the six datasets so each counts equally)
   * ------------------------------------------------------------------ */
  (function cliff() {
    ...
    const LLM = [1.0, 0.6384, 0.355, 0.239, 0.1462, 0.0355, 0.0];
    const GNN = [0.4078, 0.2293, 0.199, 0.2536, 0.2832, 0.4285, 0.7823];

    const M = { left: 46, right: 16, top: 18, bottom: 40 };
    const W = 640, H = 300;
    ...
    [0.25, 0.5, 0.75].forEach((f) => {
      svg.appendChild(el('line', { x1: M.left, y1: M.top + f * plotH, x2: W - M.right, y2: M.top + f * plotH, class: 'cliff-grid' }));
    });
    ...
    const yLbl = el('text', { x: 10, y: 14, class: 'cliff-tick' });
    yLbl.textContent = 'relative error →';
    ...
    function update() {
      ...
      if (a < 0.25) {
        message.textContent = 'Near-pure geometry. The language model is close to its worst point on the whole sweep.';
      } else if (a > 0.75) {
        message.textContent = 'Near-pure composition. This is exactly what language models are built to read.';
      } else {
        message.textContent = 'Near the balanced midpoint — both families sit closer to their own best case.';
      }
      verdict.textContent = `LLM relative error ${llmV.toFixed(2)} · GNN relative error ${gnnV.toFixed(2)}`;
    }
